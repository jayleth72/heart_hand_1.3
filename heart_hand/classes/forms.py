...
class LessonEntryForm(FlaskForm):
    ...
    lesson_time = IntegerField('Lesson Time',  validators=[InputRequired(message='Lesson time is required')])
    lesson_start_date = DateField('Payment Date', format='%Y-%m-%d', validators=[InputRequired(message='Start Date is required')])
